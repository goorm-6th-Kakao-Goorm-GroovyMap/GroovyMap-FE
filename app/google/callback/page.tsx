                ...
                try {
                    ...
                    switch (loginStatus) {
                        ...
                        case 'SAME_EMAIL':
                            ...
                            toast.error(
                                '이미 존재하는 이메일입니다. 다른 로그인 방식을 선택해주세요.'
                            );
                            router.push('/login');
                            break;
                        case 'SAME_NICKNAME':
                            // 같은 닉네임이 존재할 때 회원가입 페이지로 이동하며, 닉네임 입력란은 비워둠
                            const sameNicknameParams = new URLSearchParams({
                                email,
                                nickname: '',
                            }).toString();
                            router.push(
                                `/signup/nickname-region-part?${sameNicknameParams}`
                            );
                            break;
                        case 'NEED_REGISTER':
                            // 회원가입이 필요할 때, 회원가입 페이지로 이동
                            const needRegisterParams = new URLSearchParams({
                                email,
                                nickname,
                            }).toString();
                            router.push(
                                `/signup/nickname-region-part?${needRegisterParams}`
                            );
                            break;
                        default:
                            // 에러 처리
